...
Those 2 files should be enough to grab syslog message generated from Nokia router and transform it to proper format. To check if it is really working/matching the syslog messages we can run following command:
...
The publisher is no longer CLI and by default, the messaging format system that napalm-logs is using is zmq. You need to install pyzmq to send/receive such message. To check if your message reaches your client, just run the script from python interpreter(taken from napalm-logs):
...
First, I will shorten the name of the dir, so when I register new pack it will have shorter name - just for my convenience:
...
Changing permissions so it's similar as other packs generated by default - not sure if needed.

We also need to add config files for our pack in '/opt/stackstorm/configs/'. According to the documentation from https://github.com/StackStorm-Exchange/stackstorm-napalm_logs, napalm_logs.yaml and napalm-logs.crt. This napalm_logs.yaml name, should correspond to our pack name that we will register soon. Inside the file put this:
...
```
According to the Stackstorm documentation change ownership of the napalm_logs.yaml to be:

```
sudo chown st2:st2 napalm_logs.yaml
```

Again, those values are defaults so you may need to change those in order to receive messages published from napalm-logs. We also need to create certification_file according to our config file, so create it within config dir and leave it empty or add some comment.

```
touch napalm-logs.crt
```

We are not using certifications in this example but it's recommend option. Because we disabled security, we need to do some small changes in our sensor that sits in '/opt/stackstorm/packs/napalm_logs/sensors/napalm_logs_client.py':
...
Whenever some errors pop up either while config files, trigger or sensor registering - sensor will not work. So, while registering all you really should not see any errors. You can always isolate problems by registering part of your pack with --register-actions or triggers, rules etc.

You should see both new sensor as well as new trigger generated. Just check if st2 lists napalm_logs.NapalmLogsSensor and napalm_log.log (trigger).
Check if sensor is enabled and you should be good to receive a napalm log messages.

We have not defined rule nor action yet, but still we can verify if sensor works correctly. To do so first run napalm-logs, then generate syslog event by simulating card insertion. If message was properly consumed by sensor you should see it by running:
...
This rule grabs triggers that our sensor is generating, process it through its criteria section and whenever pattern matches the triggers, action section is run. We have not yet created action that is referenced in above example, but we will get to it shortly. You need to note that the action is triggered with parameters that were delivered from syslog message. This would be the IOM/MDA slots and actual router that the cards were inserted. Those parameters will be passed to our action that will simply take proper action.

Now, let's create './actions/pythonactions/' in my napalm_logs pack. Here is the place where action files are kept. Action yaml file is called mda_provisioning.yaml and it looks like this:
...
The python script is called syslog_mda_runner.py. and it looks like that:
...
Both should be successfully created before testing solution.

To test this example, I need to reboot my VM, as that is the only way when router will generate syslog message about card insertion. Obviously not true for real equipment and real networks.

Now when our naplam-logs are running, when sensors, rules and actions are successfully registered or created, we just need to wait for a syslog event to be generated from the router. I will reboot my VM and let's see the results...

Before checking results let me show what kind of syslog messages are generated from my VM during reboot:
...
Those are the messages that my VM is generating and theses ones should be matched against expression defined in MODULE_INSERTED.yml. It means 3 messages should be published to our client (Stackstorm sensor). Again, we can verify if trigger-instance was generated by running
